-- Update redeem_org_invite to set status to 'active' instead of 'pending'
...
BEGIN
  ...
  IF v_existing IS NOT NULL THEN
    ...
    IF v_existing.status = 'revoked' THEN
      RETURN jsonb_build_object('success', false, 'error', 'Your access to this organization has been revoked.');
    END IF;
...
-- Auto-approve existing pending members
-- This will trigger the sync to members/alumni tables because of the ON UPDATE trigger
